...
def main():
    ...
    parser.add_argument('--labelroot', type=str, default='single/', help='place to store the image-level labels and annotation')
    ...
    #实例级特征提取器 预训练权重
    parser.add_argument('--pretrain_weight', type=str, default='checkpoints/ssl/simclr/checkpoint/simclr_pretrained.pth.tar', help='pretrained weight of the SSL model')
    parser.add_argument('--pseudo_label_path', type=str, default='single/annotation/init_ins_pseudo_label.p', help='initial instance pseudo label path of SSL model')
    ...
    # comet API key
    parser.add_argument('--comet_api_key', type=str, default="Jks6ECAvi4V8ANqGk4DYxLbvt", help='comet api key')
    ...
    train_dataset_pos_full = InssepDataset(args.root_dir, 'dataset/train_bags.txt', transform,
                                           pseudo_label=args.pseudo_label_path, threshold=args.threshold,
                                           witness_rate=args.witness_rate, posbag=True,
    ...
    val_dataset = InsDataset(args.root_dir, 'dataset/val_bags.txt', transform,
                             ...
                             labelroot=args.labelroot)
    test_dataset = InsDataset(args.root_dir, 'dataset/test_bags.txt', transform, pseudo_label=None,
                              threshold=args.threshold, witness_rate=args.witness_rate, labelroot=args.labelroot)

    # get the loader for traditional loading dataset
    train_loader_pos, train_loader_neg, valid_loader, test_loader = get_train_validation_data_loaders(train_dataset_pos,
                                                                                                      train_dataset_neg,
                                                                                                      val_dataset,
                                                                                                      test_dataset,
                                                                                                      config)
    ...
    train_loader_pos_full, train_loader_neg_clean = get_train_validation_data_loaders_SPL(train_dataset_pos_full,
                                                                                          train_dataset_neg_clean,
                                                                                          config)
    ...
    trainer = Trainer((train_loader_pos, train_loader_neg, valid_loader, test_loader, train_loader_pos_full,
                       train_loader_neg_clean), config, experiment, args)
