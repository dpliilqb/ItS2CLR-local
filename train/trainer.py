...
def get_features(net, dataset):
    ...
    for b in tqdm(dataloader):
        ...
        for i, idx in enumerate(indices):
            ...
            bag_name, loc = file_name.strip('.png').rsplit('/', 1)
            embedding_dict[bag_name.lstrip('/')].append((loc, features[i]))
            # break
...
def epoch_pseudolabel(bag_ins_list, criterion, milnet, args):
    ...
    epoch_loss = 0.0
    # print("epoch_pseudolabel running")
    # with torch.no_grad():
    for i, data in enumerate(bag_ins_list):
        ...
        for ins_indx, ins_name in enumerate(data[2]):
            ins_predictions[data[3]][ins_name] = float(ins_pred[ins_indx].detach().cpu().squeeze().numpy())
...
class SimCLR(object):
    ...
    def _validate(self, model, valid_loader):
        ...
        with torch.no_grad():
            ...
            if counter == 0:
                ...
                valid_loss /= counter
                print("Validate loss calculated")
        model.train()
        return valid_loss
    ...
    def train_MIL(self, root, epoch_count, args):
        ...
        for i, b in enumerate(test_dataset):
            test_list.append([int(labels.iloc[i][1] == 'pos')
                                 , b[0].numpy(), b[2], b[3]])
        ...
        # print("eval_pseudo_label_malbag over")
    ...
    def eval_pseudo_label_malbag(self, root, ins_predictions, train=True):
        ...
        for name in train_name:
            if "pos" in name[0]:
                train_y_list.append(train_gt[name[0].strip('/')][name[1].split('.')[0]])
                # print("name", name[0])
                # print(pred_test.keys())
                if name[0] not in pred_test.keys():
                    y_prob_list.append(pred_test[name[0].strip('/')][name[1].split('.')[0]])
                else:
    ...
    def eval_pseudo_label(self, root, ins_predictions, train=True):
        ...
        pred_test = ins_predictions
        train_name = np.stack([(k, row[0]) for k, v in train_embedding.items() for row in v])
        train_y = np.array([train_gt[name[0].strip('/')][name[1].split('.')[0]] for name in train_name])
    ...
    def train(self):
        ...
        if not os.path.exists(model_checkpoints_folder):
            model_checkpoints_path = os.path.normpath(model_checkpoints_folder)
...
def load_pretrained_simCLR(net, model_dir):
    ...
    print('load pretrained simCLR successfully')
...
def load_pretrained_other(net, model_dir):
    ...
    print('load pretrained other successfully')
